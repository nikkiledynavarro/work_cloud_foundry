...
sap.ui.define([
	...
	return BaseController.extend("com.erpis.shiperp.parceldemo.hd6.controller.Main", {
		...
		oFreightUnitHazmatItems: {}, // The hashmap (which key: HU number, value: hazmatlist) contain the hazmat which is already loaded.
		...
		onAfterRateAnalysisOpen: function () {
			...
			try {
				...
				this._oLogger.info("No Carrier Rate Analysis");
			}
		},
		...
		onChangeScaleOption: function (oEvent) {
			if (oEvent.getSource().getSelectedKey() === "0001") {
				...
				this.getModel("local").setProperty("/aFreightUnitEdits", []);
			} else if (oEvent.getSource().getSelectedKey() === "0003") {
		...
		onChangeSerialMultiInput: function (oEvent) {
			...
			// oControl.attachEventOnce("change", this.addValidatorForScan, this);

			// this.addValidatorForScan(oEvent);
		},
		...
		onHazardousClose: function () {
			...
			var oBinding = oSelectedHu.getBindingContext("local");
			var sPath = oBinding.getPath();
			var sHazmatPath = sPath + "/FreightunitHazmat";
			this.getModel("local").setProperty(sHazmatPath, []);
			var sKey = oSelectedHu.getBindingContext("local").getObject().freightunitkey;
			delete this.oFreightUnitHazmatItems[sKey];
		...
		onAdvancedPackingDialogAfterOpen: function (oEvent) {
			this._refreshPackingData();
		},
		...
		onChangeHUHeaderEditableFields: function (oEvent) {
			...
			this._updateHUAdvancedPackingDimensions();
		},
		...
		_generateGetContentHUUsecase: function (sWhichTable) {
			...
			// Prepare FreightUnit table json
			var sMPSStatus = this.getModel("local").getProperty("/basic/carrier_data/mps/mps");
			var aFreightUnits = [];
			var sMPSType = this.getModel("local").getProperty("/basic/carrier_data/mps/mpstype");
			if (sMPSStatus === "X" && sMPSType === "02") {
			...
			for (var i = 0; i < aFreightUnits.length; i++) {
				...
				if (aFreightUnits[i].FreightunitHazmat.results == null) {
					aFreightUnits[i].FreightunitHazmat = [];
				}
			}

			// Prepare Content table json
			var aContents = this.getModel("local").getProperty("/Contents");
			aContents.forEach(function (item) {
				item.partialqty = item.partialqty.toString();
			});
			var oData = {
		...
		_getHazardous: function () {
			...
			this.getModel().create("/ShipmentQuerySet", oRequestData, {
				success: function (oData) {
					...
					this.oFreightUnitHazmatItems[sKey] = aResults;
		...
		_getLocalFreightUnitHazmatItems: function (oSelectedFreightUnitItem) {
			...
			var aResults = [];
			var aHazmatContinues = oSelectedFreightUnitItem.FreightunitHazmat;
			aResults = this.oFreightUnitHazmatItems[sKey] || [];
				...
				success: function () {
					...
					for (var i = 0; i < this.getModel("local").getProperty("/FreightunitHazmats").length; i++) {
						...
						if (oItem.Selected === "X" && oItem.Updated === "X") {
							aHazmat.push(oItem);
							// Set the freight unit indicator to green once the validation is done
							oSelectedHu.setHighlight("Success");
							oSelectedHu.setTooltip("Hazardous material has been updated successfully");
						}
					}
		...
		_updateHUAdvancedPackingDimensions: function() {
			var oRequestData = this._generateUpdateHUFieldsUsecase();
			this.showBusy();
			this.getModel().create("/ShipmentQuerySet", oRequestData, {
				success: function (oData) {
		...
		_generateUpdateHUFieldsUsecase: function () {
