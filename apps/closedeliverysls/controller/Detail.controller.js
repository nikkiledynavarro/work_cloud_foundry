/*global location */
sap.ui.define([
	"com/erpis/shiperp/closedelivery/hr7/controller/BaseController",
	"sap/ui/model/json/JSONModel",
	"com/erpis/shiperp/closedelivery/hr7/model/formatter",
	"sap/ui/Device",
	"com/erpis/shiperp/closedelivery/hr7/common/Utils",
	"sap/m/MessageBox",
	"sap/m/MessageToast",
	...
	return BaseController.extend("com.erpis.shiperp.closedelivery.controller.Detail", {
		...
		_onObjectMatched: function (oEvent) {
			...
			this.getModel().metadataLoaded().then(function () {
				...
				//Axo 4852
				// var sObjectPath = this.getModel().read("SalesOrderListSet?$filter=SalesNo eq '" + this.sSalesNo + "'");
		...
		_onBindingChange: function () {
			...
			var saleNo = oElementBinding.getBoundContext().getObject().SalesNo; //(+) Axo 3852 Tim update 20/12/2021
			// if (!saleNo) { // (+) Axo 3852 Tim update 20/12/2021
			if (!oElementBinding.getBoundContext()) { //{(-) Axo 3852 Tim update 20/12/2021
				return;
			} else {
				// Bind Shipset table
				if (this.getModel("global").getProperty("/dontRefresh")) {
					...
					// var sDeliblockPath = oElementBinding.getPath(); + "/to_deliBlock"; (-) Axo 4852 Tim update 20/12/2021
					var sDeliblockPath = "/SalesOrderOBDStatusSet"; // (+) Axo 4852 Tim update 20/12/2021
					var oCreatedOn = this.getMasterCreatedOnData();
					var oDateFormat = sap.ui.core.format.DateFormat.getDateInstance({
						pattern: "yyyyMMdd"
					});
					var sDateFrom = oDateFormat.format(new Date());
					var sDateTo = oDateFormat.format(new Date());
					if (oCreatedOn && oCreatedOn.from) {
						sDateFrom = oDateFormat.format(oCreatedOn.from);
						sDateTo = oDateFormat.format(oCreatedOn.to);
					}
					var aFilters = [
						new Filter("SalesNo", "EQ", this.sSalesNo),
						new Filter("CreatedOn", "BT", sDateFrom, sDateTo)
					];

					this.getModel().read(sDeliblockPath, {
						filters: aFilters,
											...
											new sap.m.List({
												items: {
													...
													template: new sap.m.StandardListItem({
														title: "{DelivBlockDesc}" //DelivBlockDesc Axo 4852 Tim update 20/12/2021
													})
												}
											})
										]
		...
		onShowRatePricingDetail: function () {
			...
			var oRatePricingTemplate = sap.ui.xmlfragment("com.erpis.shiperp.salesorder.hr7.fragment.RatePricingColumnListItem", this);
			var oBindingInfo = {
		...
		onShowRateDetail: function () {
			...
			var oObject = this.byId("tableRates").getSelectedItem().getBindingContext("global").getObject();
			var oRateDetailTemplate = sap.ui.xmlfragment("com.erpis.shiperp.salesorder.hr7.fragment.RateDetailColumnListItem", this);
			var oBindingInfo = {
		...
		onValidateAddressConfirmPress: function () {
			...
			//(-)Axo 4856 Tim Updated 20/12/2021
			...
			// (+) Axo 4856 Tim Updated 20/12/2021
			this.getModel().setProperty(sPath + "/ShipToParty/city", OldAddress.Ort01);
			this.getModel().setProperty(sPath + "/ShipToParty/country", OldAddress.Land1);
			this.getModel().setProperty(sPath + "/ShipToParty/name1", OldAddress.Name1);
			this.getModel().setProperty(sPath + "/ShipToParty/postalCode", OldAddress.Pstlz);
			this.getModel().setProperty(sPath + "/ShipToParty/region", OldAddress.Regio);
			this.getModel().setProperty(sPath + "/ShipToParty/street", OldAddress.Stras);

			this.getModel("global").setProperty("/SalesOrderDetail/ShipToParty/city", OldAddress.Ort01);
			this.getModel("global").setProperty("/SalesOrderDetail/ShipToParty/country", OldAddress.Land1);
			this.getModel("global").setProperty("/SalesOrderDetail/ShipToParty/name1", OldAddress.Name1);
			this.getModel("global").setProperty("/SalesOrderDetail/ShipToParty/postalCode", OldAddress.Pstlz);
			this.getModel("global").setProperty("/SalesOrderDetail/ShipToParty/region", OldAddress.Regio);
			this.getModel("global").setProperty("/SalesOrderDetail/ShipToParty/street", OldAddress.Stras);
		},
				...
				success: function (oData) {
					...
					if (oData.ShipSetSet) {
						...
						for (var i = 0; i < aShipsetResults.length; i++) {
							...
							//added by Tim 28/9/2021
							if (!aShipsetResults[i].MoreOptionSet) {
								aShipsetResults[i].MoreOptionSet = [];
							}
						}
		...
		_generateGetShipsetUsecase: function () {
			var oData = {
				...
				ShipSetSet: [{
					...
					MoreOptionSet: [{
							MoreOptionItemSet: []
						}] //added by Tim 28/9/2021 to get carrier moreopt data
				}],
				SOActive: {},
				AddrVal: {},
				// New Structure
