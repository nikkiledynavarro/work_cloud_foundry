sap.ui.define([], function () {
	"use strict";
	var sNameSpace = "com.erpis.shiperp.manualshipmentewm.hr7.fragment.";
	return {
		...
		_addPrefixToFieldName: function (aSource, prefix) {
			for (var i = 0, len = aSource.length; i < len; i++) {
				aSource[i].field_name = aSource[i].field_name.replaceAll(prefix, "");
				aSource[i].field_name = prefix + aSource[i].field_name;
				if (aSource[i].field_pname !== "") {
					aSource[i].field_pname = aSource[i].field_pname.replaceAll(prefix, "");
					aSource[i].field_pname = prefix + aSource[i].field_pname;
				}
