...
sap.ui.define([
	...
	return BaseController.extend("com.erpis.shiperp.trackshipmentewm.hr7.controller.ECC.Main", {
		...
		onInit: function () {
			...
			this.getRouter().getRoute("mainECC").attachPatternMatched(this._onObjectMatched, this);
		...
		_onObjectMatched: function (oEvent) {
			...
			//carrier performance report app
			this.sTrackShipment = oEvent.getParameter("arguments").TrackShipment;
			...
			if (sTrackno) {
				this.byId("cbInputType").setSelectedKey("TN"); //Tracking Number
			...
			this.byId("txtIdECC").setEditable(true);
			if (this.sTrackShipment) {
				this.byId("cbInputType").setSelectedKey("TN") //Tracking Number
				this._getTrackingData(this.sTrackShipment);
			} else {
				this.hideBusy();
			}
		},
		...
		/* event handlers                                              */
		/* =========================================================== */
		_getTrackingData: function (TrackShipment) {
			this.showBusy();
			this.byId("txtIdECC").setValue(TrackShipment);
			this.byId("txtIdECC").setEditable(false);
			this.sInputType = this.byId("cbInputType").getSelectedKey();
			var oRequestData = this._generateGetTrackDataUsecase(TrackShipment);
			this.getModel("ECCService").create("/TrackShipmentReturnSet", oRequestData, {
				success: function (oData) {
					...
					if (oData.MultiDelivery !== null) {
						if (oData.MultiDelivery.ShowMultiDeliv === 'X') {
						...
						this.oValidateTrackDialog.open();
						sap.ui.getCore().byId("idTrack").removeSelections();
						this.hideBusy();
					}
		...
		onSubmit: function (oEvent) {
			...
			this.showBusy();
			var oRequestData = this._generateGetTrackDataUsecase(this.sInputID);
			this.getModel("ECCService").create("/TrackShipmentReturnSet", oRequestData, {
				success: function (oData) {
					this.getModel("local").setProperty("/TrackData", oData.TrackShipmentNew.results);
					this.getModel("local").setProperty("/TrackReply", oData.TrackReply.results);
		...
		onCancel: function () {
			this.oValidateTrackDialog.close();
			var oTableList = sap.ui.getCore().byId("idTrack").getSelectedItems();
		...
		_generateTrackSelectedDataUsecase: function () {
			var oData = {
				...
				MultiDelivery: {
					ShowMultiDeliv: "",
					Deliveries: this.getModel("local").getProperty("/aTrackShimentList")
				}
			};
			return oData;
		},

		onTrackShipment: function (oEvent) {
			var oTableList = sap.ui.getCore().byId("idTrack").getSelectedItems();
			var aTrack = oEvent.getSource().getModel("local").getProperty("/aTrackShimentList");
			for (var i = 0; i < oTableList.length; i++) {
				for (var j = 0; j < aTrack.length; j++) {
					var oSelectionField = oTableList[i].getBindingContext("local").getObject();
					if (oSelectionField.Delivery === aTrack[j].Delivery) {
						aTrack[j].SelectionField = "X";
					} else {
						aTrack[j].SelectionField = "";
					}
				}
			}
			this.getModel("local").setProperty("/aTrackShimentList", aTrack)
			var oRequestData = this._generateTrackSelectedDataUsecase();
			this.showBusy();
			this.getModel("ECCService").create("/TrackShipmentReturnSet", oRequestData, {
				success: function (oData) {
					this.getModel("local").setProperty("/TrackData", oData.TrackShipmentNew.results);
		...
		_generateGetTrackDataUsecase: function (sInputID) {
			var oData = {
				Profile: (this.sProfile) ? this.sProfile : "",
				Station: (this.sStation) ? this.sStation : "",
				InputType: (this.sInputType) ? this.sInputType : "",
				InputId: (sInputID) ? sInputID : "",
				TrackReply: [],
				TrackShipmentNew: [],
				MultiDelivery: {
					ShowMultiDeliv: "",
		 ...
		 */
		_generateGetTrackDataDetailUsecase: function (sTrackNumber) {
			delete sTrackNumber.__metadata;
			var oData = {
				Profile: this.sProfile,
				Station: this.sStation,
				InputType: this.sInputType,
		...
		_builData: function (sTrackNumber) {
			var oDeferred = $.Deferred();
			var oRequestData = this._generateGetTrackDataDetailUsecase(sTrackNumber);
			this.showBusy();
			this.getModel("ECCService").create("/TrackShipmentReturnSet", oRequestData, {
				success: function (oData) {
					this.getModel("local").setProperty("/TrackData", oData.TrackShipmentNew.results);
		...
		onOpenTrackingStatusDialog: function (oEvent) {
			...
			var sTrackNumber = oEvent.getSource().getBindingContext("local").getObject();
			oDeferred = this._builData(sTrackNumber);
		...
		onSearchTrack: function (oEvent) {
			...
			sap.ui.getCore().byId("idTrack").getBinding("items").filter([oAllFilter]);
		},
		// end
	});
});