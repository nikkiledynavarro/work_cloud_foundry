/*global location*/
jQuery.sap.require("com.erpis.shiperp.manualshipmentecc.common.jquery_hotkeys");
sap.ui.define([
	"com/erpis/shiperp/manualshipmentecc/controller/BaseController",
	"sap/ui/model/json/JSONModel",
	"com/erpis/shiperp/manualshipmentecc/model/formatter",
	"sap/m/Token",
	"sap/ui/model/Filter",
	"sap/m/MessageBox",
	"sap/m/MessageToast",
	"com/erpis/shiperp/manualshipmentecc/common/Utils",
	"com/erpis/shiperp/manualshipmentecc/common/hotkeyInterface",
	"com/erpis/shiperp/manualshipmentecc/common/DynamicView",
	"com/erpis/shiperp/manualshipmentecc/common/UploadUtils",
	"com/erpis/shiperp/manualshipmentecc/common/HttpHelper"
], function (BaseController, JSONModel, formatter, Token, Filter, MessageBox, MessageToast, Utils, HotkeyInterface, DynamicView,
	UploadUtils, HttpHelper) {
	"use strict";

	return BaseController.extend("com.erpis.shiperp.manualshipmentecc.controller.Main", {

		oLogger: jQuery.sap.log.getLogger("com.erpis.shiperp.manualshipmentecc.controller.Main"),
		formatter: formatter,
		...
		_onObjectMatched: function (oEvent) {
			...
			// check manual app ECC
			this.checkmaualapp = "M";
			// load data infor
			this.byId("iconParcel").setBusy(true);
			...
			var oDeferredData = this._ScanDataLogin(this.sStation, this.sProfile, this.sWarehouseNumber);
			$.when(oDeferredData, this.oInputTypeDeferred).done(function () {
				this.hideBusy();
			}.bind(this));
		},
		...
		onSubmitShipTo: function (oEvent) {
			...
			if (this.byId("cbShippingPo").getSelectedKey() !== "") {
				this.shippingpoint = this.byId("cbShippingPo").getSelectedKey();
				this.byId("cbShippingPo").setValueState("None");
			}
			// else {
			// 	MessageBox.error(this.oBundle.getText("missingInputType"));
			// 	this.byId("cbShippingPo").setValueState("Error");
			// 	return;
			// }
			var oShiptos = oEvent.getSource().getValue();
			if (oShiptos === "") {
			...
			this._ScanShipTo(oShiptos);
		},
		_ScanDataLogin: function (oValue) {
			var oDeferred = $.Deferred();
			var sPath = "/ParcelQuerySet";
			this.showBusy();
			this.getModel().read(sPath, {
				filters: [
					new Filter("input/input_ids", "EQ", this.sInputIDs),
					new Filter("input/input_type", "EQ", this.sInputType),
					new Filter("input/profile", "EQ", this.sProfile),
					new Filter("input/shippingstation", "EQ", this.sStation),
				],
				urlParameters: {
					"$expand": "carrier_more_option/value_list,Freightunits/carrier_more_option/value_list,Freightunits/FreightunitItems,SerialSet,SerialSet/SerialItemSet,HTS,References,CarrierList,ServiceList,Contents,NaftaDetailSet"
				},
				success: function (oData) {
					this.getModel("local").setProperty("/oManualShipment", this.oManualShipment);
					//hide load display
					this.byId("iconParcel").setBusy(false);
					...
					if (oData.results.length !== 0 && oData.results[0].basic.carrier_data.carrier !== "") {
						...
						this.shippingPoint = oData.results[0].input.shippingPoint;
						this.byId("cbShippingPo").setValue(this.shippingPoint);
						...
						// set selectedkey shippingPoint
						var sInput = oData.results[0].input;
						this.byId("cbShippingPo").setValue(sInput.shippingPoint);
						...
						// No Disable inputs fields after scan login
		...
		_ScanShipTo: function (oShiptos) {
			this.shiptos = oShiptos;
			this.shippingpoint = this.byId("cbShippingPo").getValue();
			...
			this.getModel().read(sPath, {
				...
				urlParameters: {
					"$expand": "carrier_more_option/value_list,International/value_list,Freightunits/carrier_more_option/value_list,Freightunits/FreightunitItems,SerialSet,SerialSet/SerialItemSet,HTS,References,CarrierList,ServiceList,Contents,NaftaDetailSet"
				},
				success: function (oData) {
					if (oData.results.length !== 0 && oData.results[0].basic.carrier_data.carrier !== "") {
						this.getModel("local").setProperty("/ShipmentCarrierOptions", oData.results[0].carrier_more_option.results);
		...
		onAddDefaultHU: function () {
			if (!this.oDialogDefaultHU) {
				this.oDialogDefaultHU = sap.ui.xmlfragment("com.erpis.shiperp.manualshipmentecc.fragment.CreateDefaultHUDialog", this);
		...
		onAddNewHU: function () {
			if (!this.oDialogNewHU) {
				this.oDialogNewHU = sap.ui.xmlfragment("com.erpis.shiperp.manualshipmentecc.fragment.CreateHUDialog", this);
			...
			MessageBox.confirm(this.oBundle.getText("confirmDeleteHUMessage"), {
				...
				onClose: function (oAction) {
					if (oAction === MessageBox.Action.YES) {
						var aSelected = this.byId("tableHU").getSelectedItems();
						var aDataSelected = [];
						var aDeletData;
						if (aSelected.length > 0) {
							...
							aDataSelected.forEach(function (element2) {
								aDeletData = aHUs.filter(function (element1) {
									return !(element1.freightunitkey === element2.freightunitkey);
								});
							});
							this.getModel("local").setProperty("/Freightunits", aDeletData);
		...
		// Press on Reference button on Small manualecc tab
			...
			/*
																...
																if (oFreightUnit.isOverviewPack) {
																	...
																	if (!this._oHUItemsDialog) {
																		this._oHUItemsDialog = sap.ui.xmlfragment("com.erpis.shiperp.manualshipmentecc.fragment.packing.HUItemsDialog", this);
		...
		_generateMainShipToUsecase: function (sValue) {
			var sActionName = "CheckManShipTo";
			var oData = {
				transaction_class: this.checkmaualapp,
		...
		onCheckManShipTo: function (oEvent) {
			var oControl = oEvent.getSource();
			var sValue = oControl.getValue();
			var oRequestData = this._generateMainShipToUsecase(sValue);
			this.showBusy();
			this.getModel().create("/ShipmentQuerySet", oRequestData, {
				success: function (oData) {
					if (oData.shipto !== "") {
						var oInputShipTo = this.byId("cbInputTypeShipTo");
						oInputShipTo.setValue(oData.shipto);
						if (oData.ReturnMessages.results.length > 0) {
							var aMsg = this._generateMessagess(oData.ReturnMessages.results);
							var string = aMsg[0].title;
							MessageBox.warning(string);
						}
						var oDeferredScan = this._ScanShipTo(oData.shipto);
						$.when(oDeferredScan).done(function () {
							this.hideBusy();
						}.bind(this));
					}
					this.hideBusy();
		...
		_getINTL: function () {
			var aTextDelivery = [];
			var aDelivery = this.byId("cbInputTypeShipTo").getValue();
			aTextDelivery.push(aDelivery);
			var sShipToNumber = aTextDelivery.toString();
			...
			var oData = {
				ShipToNumber: sShipToNumber,
				Carrier: sCarrier,
				ShipToCountry: sShipToCountry,
				ShipFromCountry: sShipFromContry,
				TransactionClass: this.checkmaualapp
			};
			return oData;
		},
		...
		onShowPricingDetail: function () {
			...
			if (!this.oRatePricingDialog) {
				this.oRatePricingDialog = sap.ui.xmlfragment("com.erpis.shiperp.manualshipmentecc.fragment.RatePricingsDialog", this);
			...
			var oRatePricingTemplate = sap.ui.xmlfragment("com.erpis.shiperp.manualshipmentecc.fragment.RatePricingColumnListItem", this);
			var oBindingInfo = {
		...
		onShowRateDetail: function () {
			...
			if (!this.oRateDetailDialog) {
				this.oRateDetailDialog = sap.ui.xmlfragment("com.erpis.shiperp.manualshipmentecc.fragment.RateDetailsDialog", this);
				this.getView().addDependent(this.oRateDetailDialog);
			}
			var oObject = sap.ui.getCore().byId("tableRates").getSelectedItem().getBindingContext("local").getObject();
			var oRateDetailTemplate = sap.ui.xmlfragment("com.erpis.shiperp.manualshipmentecc.fragment.RateDetailColumnListItem", this);
			var oBindingInfo = {
		...
		onRateSelected: function () {
			...
			// Update basic node from new carrier
			this._changeCarrierForRate((oRate.serviceid) ? oRate.serviceid : "");
		},
		...
		onSerialPress: function () {
			if (!this.oSerialDialog) {
				this.oSerialDialog = sap.ui.xmlfragment("com.erpis.shiperp.manualshipmentecc.fragment.SerialDialog", this);
		...
		_getMessagePopover: function () {
			if (!this._messagePopover) {
				this._messagePopover = sap.ui.xmlfragment("com.erpis.shiperp.manualshipmentecc.fragment.ValidateAddressDialog", this);
			}
		...
		onHazmatAddItemPress: function () {
			...
			var bcount = aFreightunitHazmats.length + 1;
			oNewHazmat.linenumber = bcount.toString();
		...
		onPCDGunCBChange: function (oEvent) {
			...
			var sHothazmatsubclas = oControl.getSelectedItem().getBindingContext().getObject().dothazmatsubclas;
			var sPackinggroup = oControl.getSelectedItem().getBindingContext().getObject().packinggroup;
			var sDothazmatclass = oControl.getSelectedItem().getBindingContext().getObject().dothazmatclass;
			...
			this.getModel().callFunction("/GetSingleHazmat", {
				urlParameters: {
					HazmatID: sKey,
					Dothazmatsubclas: sHothazmatsubclas,
					Packinggroup: sPackinggroup,
					Dothazmatclass: sDothazmatclass,
		...
		HarmonizedClose: function (oEvent) {
			oEvent.getSource().getBinding("items").filter([]);
		},
		...
		HarmonizedValueHelpSearch: function (oEvent) {
			var sValue = oEvent.getParameter("value");
			var oFilterID = new Filter("harmonizedcode", sap.ui.model.FilterOperator.Contains, sValue);
			var oFilterValue = new Filter("Text1", sap.ui.model.FilterOperator.Contains, sValue);
		...
		onHarmonizedConfirm: function (oEvent) {
			var oSelectedItem = oEvent.getParameter("selectedItem");
			if (oSelectedItem) {
				this.sPathContents.setValue(oSelectedItem.getTitle());
			}
			oEvent.getSource().getBinding("items").filter([]);
		},
		...
		onReprint: function () {
			...
			var oReprintTemplate = sap.ui.xmlfragment("com.erpis.shiperp.manualshipmentecc.fragment.ReprintColumnListItem", this);
			// var oFilter
		...
		onShowFDXGHoldLocationDetail: function () {
			...
			if (!this.oDialogFDXGHoldLocationDetail) {
				this.oDialogFDXGHoldLocationDetail = sap.ui.xmlfragment(
					"com.erpis.shiperp.manualshipmentecc.fragment.carrier.FedExGroundHoldAtLocationDetailDialog",
					this);
		...
		onShowFDXEHoldLocationDetail: function () {
			...
			if (!this.oDialogFDXEHoldLocationDetail) {
				this.oDialogFDXEHoldLocationDetail = sap.ui.xmlfragment(
					"com.erpis.shiperp.manualshipmentecc.fragment.carrier.FedExHoldAtLocationDetailDialog",
					this);
		...
		onShowFDXGCODRecipientDetail: function () {
			...
			if (!this.oDialogCODFDXGRecipientDetail) {
				this.oDialogCODFDXGRecipientDetail = sap.ui.xmlfragment(
					"com.erpis.shiperp.manualshipmentecc.fragment.carrier.FedExGroundCODRecipientDetailDialog",
					this);
		...
		onShowFDXECODRecipientDetail: function () {
			...
			if (!this.oDialogCODFDXERecipientDetail) {
				this.oDialogCODFDXERecipientDetail = sap.ui.xmlfragment(
					"com.erpis.shiperp.manualshipmentecc.fragment.carrier.FedExCODRecipientDetailDialog",
					this);
		...
		_resetData: function () {
			...
			this.byId("iconTabImporter").setVisible(false);
			// Enable inputs fields
			this.byId("cbShippingPo").setEditable(true);
		},
			...
			this.getModel().create("/ShipmentQuerySet", oRequestData, {
				success: function (oData) {
					...
					if (!this.oRateAnalysisDialog) {
						this.oRateAnalysisDialog = sap.ui.xmlfragment("com.erpis.shiperp.manualshipmentecc.fragment.RateAnalysisDialog", this);
		...
		_getFreightCost: function (sWhichAction) {
			...
			this.getModel().create("/ShipmentQuerySet", oRequestData, {
				success: function (oData) {
					...
					if (!this.oRateDialog) {
						this.oRateDialog = sap.ui.xmlfragment("com.erpis.shiperp.manualshipmentecc.fragment.RatesDialog", this);
		...
		_generateGetFreightCostUsecase: function (sWhichAction) {
			...
			//updated 05/06/2023 by Michael for remove prefix
			...
			this.getModel().create("/ShipmentQuerySet", oRequestData, {
				success: function (oData) {
					...
					if (oData.Freightunits) {
						...
						var aFreightunits = (this.getModel("local").getProperty("/Freightunits")) ? this.getModel("local").getProperty(
							"/Freightunits") : [];
						for (var i = 0; i < oData.Freightunits.results.length; i++) {
							if (aFreightunits.length > 0) {
								var foundFreightUnit = aFreightunits.find(function (itemFound) {
									return itemFound.freightunitkey == oData.Freightunits.results[i].freightunitkey;
								});
								if (!foundFreightUnit) {
									aFreightunits.push(oData.Freightunits.results[i]);

								}
								// for (var j = 0; j < aFreightunits.length; j++) {
								// 	if (aFreightunits[j].freightunitkey !== oData.Freightunits.results[i].freightunitkey) {
								// 		aFreightunits.push(oData.Freightunits.results[i]);
								// 	} else if (oData.Freightunits.results[i].trackingnumber !== "") {
								// 		aFreightunits = oData.Freightunits.results;
								// 	}
								// }
							} else {
								aFreightunits.push(oData.Freightunits.results[i]);
		...
		_generateGetContentHUUsecase: function (sWhichTable) {
			...
			if (this.getModel("local").getProperty("/CreateHUFreightunits")) {
				aFreightunits = this.getModel("local").getProperty("/CreateHUFreightunits");
			} else {
				aFreightunits = this.getModel("local").getProperty("/Freightunits");
			}
			...
			var oData = {
				...
				action: sActionName,
				basic: oBasic,
				Freightunits: (aFreightunits) ? aFreightunits : [],
				Contents: this.getModel("local").getProperty("/aManshipContents"),
				HTS: this.getModel("local").getProperty("/HTS"),
				References: this.getModel("local").getProperty("/References"),
				CarrierRates: []
			};
		...
		onAddProductDetail: function () {
			...
			this.getModel("local").setProperty("/AddContentsDetail", oContents);
			this.oAddProductDetailDialog = Utils.getFragment("", "packing.AddContentsDetail", this);
			this.oAddProductDetailDialog.open();
		},

		onChangeContry: function (oEvent) {
			var oController = oEvent.getSource();
			var sValue = oController.getValue();
			oController.setValue(sValue);
		},

		onshowharmonizedcode: function (oEvent) {
			this.sPathContents = oEvent.getSource();
			var sPath = this.sPathContents.getBindingContext("local").getPath();
			var sCountry = this.getModel("local").getProperty(sPath);
			var oThis = this;
			var sPath = "/xSERPERPxI_STAWN";
			this.showBusy();
			if (sCountry.ctryoforigin === "") {
				this.getModel().read(sPath, {
					success: function (oData) {
						if (oData) {
							oThis.getModel("local").setProperty("/ListHarmonizedcode", oData.results);
							var oHarmonizedcode = Utils.getFragment("", "packing.Harmonizedcode", this);
							oHarmonizedcode.open();
						}
						this.hideBusy();
					}.bind(this),
					error: function () {
				...
				this.getModel().read(sPath, {
					filters: [
						new Filter("Land1", "EQ", sCountry.ctryoforigin)
					],
					success: function (oData) {
						if (oData) {
							oThis.getModel("local").setProperty("/ListHarmonizedcode", oData.results);
		...
		onProductionDetailPress: function () {
			...
			var oAddProductDetail = this.getModel("local").getProperty("/AddContentsDetail"); // detail
			...
			if (aCheckVariable.length > 0) {
				var maxLineObject = aManshipContents.reduce(function (prev, current) {
					return (prev.deliveryitem > current.deliveryitem) ? prev : current;
				});
				ItemValue = parseInt(maxLineObject["deliveryitem"], 10);
			} else {
				ItemValue = 0;
			}

			// Add new value with current value of "deliveryitem" field
			var NewItems = ItemValue + 10;
			// Convert the result of the addition to a 6-character string format and update the value of the "deliveryitem" field again
			var sItems = NewItems.toString().padStart(6, '0');
			var arrProductDetail = [oAddProductDetail];
			arrProductDetail[0]["deliveryitem"] = sItems;
						...
						if (aSelected.length > 0) {
							...
							aDataSelected.forEach(function (element2) {
								aDeletData = apContents.filter(function (element1) {
									return !(element1.deliveryitem === element2.deliveryitem);
								});
							});
		...
		// Update basic node from new carrier
		_changeCarrierForRate: function (sServiceID) {
			// Update Carrier name and service id
			var sCarrierName = this.byId("cbCarrier").getSelectedItem().getText();
			this.getModel("local").setProperty("/basic/carrier_data/CarrierName", sCarrierName);
			this.getModel("local").setProperty("/basic/carrier_data/service", sServiceID);
		...
		_generateChangeCarrierUsecase: function () {
			...
			var oData = {
				...
				// References: this.getModel("local").getProperty("/References"),
				References: [],
				Freightunits: this.getModel("local").getProperty("/Freightunits") //added on 10/8/2021 by Tim to update carrier more option data
			};
			return oData;
		},
		...
		_createHU: function () {
			...
			this.getModel().callFunction("/CreateHU", {
				...
				success: function (oData) {
					if (oData.results.length > 0) {
						// change the structure to be the same as the freightunit structure
						var aDataFreightunit = [];
						oData.results.forEach(function (item) {
							var Freightunits = {
								shipmentid: item.shipmentid,
								hu_id: item.hu_id,
								freightunitkey: item.freightunitkey,
								packmat: item.packmat,
								weight: item.weight,
							...
							aDataFreightunit.push(Freightunits);
						});
						this.getModel("local").setProperty("/CreateHUFreightunits", aDataFreightunit);
					}
					this._refreshPackingData();
		...
		_generatePackMaterialUsecase: function () {
			...
			var aContents = [];
			var sTargetHU;
			if (this.oHUTable.getSelectedItems().length > 0) {
				sTargetHU = this.oHUTable.getSelectedItems()[0].getBindingContext("local").getObject().freightunitkey;
			} else {
				sTargetHU = this.oHUTable.getSelectedItems();
			}
			for (var i = 0; i < aSelectedItems.length; i++) {
				aSelectedItems[i].getBindingContext("local").getObject().partialqty = aSelectedItems[i].getBindingContext("local").getObject().partialqty +
					"";
		...
		_execute: function (sActionName, iValue, aManualtrack, flag) {
			...
			this.getModel().create("/ShipmentQuerySet", oRequestData, {
				success: function (oData, oResponse) {
					...
					// 	if (oData.Custom_Fields.ZShowAddrval === "X") {
					// 		if (oData.ReturnMessages.results.length > 0) {
					// 			var aMsg = this._generateMessagess(oData.ReturnMessages.results);
					// 			this.getModel("local").setProperty("/aMessagesValidateShipment", aMsg);
					// 		}
					// 		if (!this.oShipmentAddrValid) {
					// 			this.oShipmentAddrValid = sap.ui.xmlfragment("com.erpis.shiperp.manualshipmentecc.fragment.ShipmentAddressValidation", this);
					...
					// 		this.getModel("local").setProperty("/CreateHUFreightunits", oData.Freightunits.results);
					// 	}
					// }
					...
					if (oData.showmanualtrack) {
						...
						if (!this.oTrackNumbersDlg) {
							this.oTrackNumbersDlg = sap.ui.xmlfragment("com.erpis.shiperp.manualshipmentecc.fragment.tracking.TrackingNumbers", this);
					...
					if (oData.showProNum) {
						if (!this.oDialogLTL) {
							this.oDialogLTL = sap.ui.xmlfragment("com.erpis.shiperp.manualshipmentecc.fragment.CreateProNumber", this);
		...
		_generateExecuteUsecase: function (sActionName, iValue, aManualtrack, flag) {
			...
			var aContents = this.getModel("local").getProperty("/aManshipContents");
			...
			var oData = {
				...
				Contents: (aContents) ? aContents : [],
				ReturnMessages: [],
				// Custom_Fields: aCustomeFileds,
		...
		_filterAllDropdowns: function () {
			...
			*/
			...
			//  Michael back up for international tab
			/////// INTERNATIONAL TAB ///////
		...
		_displayTabs: function () {
			...
			var sCountry = this.getModel("local").getProperty("/basic/partners/shipto/address/country");
			if (sCountry !== "") {
				this.byId("btnHTS").setVisible(true);
			} else {
				this.byId("btnHTS").setVisible(false);
			}
			//check for international tabs
			if (bDomestic) {
				this.byId("iconTabInternational").setVisible(false);
				this.byId("iconTabImporter").setVisible(false);
			}
			// check for HTS button
			if (!bDomestic) {
				this.byId("iconTabInternational").setVisible(true);
				this.byId("iconTabImporter").setVisible(true);
			}
		...
		_displayShipmentCarrierMoreOptTab: function () {
			...
			var aShipmentCarrierMoreOptions = this.getModel("local").getProperty("/ShipmentCarrierOptions");
		...
		_getHazardous: function () {
			...
			this.getModel().create("/ShipmentQuerySet", oRequestData, {
				success: function (oData) {
					var aFreightunitHazmat = [];
					var aResults = oData.Freightunits.results;
					aResults.forEach(function (item) {
						if (item.FreightunitHazmat.results.length > 0) {
							item.FreightunitHazmat.results.forEach(function (itemHaz) {
								aFreightunitHazmat.push(itemHaz);
							});
						}
					});
					this.getModel("local").setProperty("/FreightunitHazmats", aFreightunitHazmat);
		...
		_generateUpdateHUFieldsUsecase: function (bHeader) {
			...
			var oData = {};
			var aFreightUnit = this.getModel("local").getProperty("/Freightunits");
			var aDataFreightunit = [];

			aFreightUnit.forEach(function (item) {
				var Freightunits = {
					hu_id: item.hu_id,
					freightunitkey: item.freightunitkey,
					packmat: item.packmat,
		...
		_refreshPackingData: function (bIgnoreEditableFields) {
			...
			if (this._getPackingScenario() === "01") { // Main Screen Scenario
				if (bIgnoreEditableFields === undefined) {
					// Update editable fields in the backend
					// oDeferred = this._updateHUAdvancedPackingDimensions(false);
					oDeferred = this._getContentAndHUTable("All");
					$.when(oDeferred).done(function () {
						// this._getContentAndHUTable("All");
						oDeferred = $.Deferred();
					}.bind(this));
		...
		onSelectCommCodeService: function (oEvent) {
			...
			var oBasic = this.getModel("local").getProperty("/basic");
			this.shippingpoint = this.byId("cbShippingPo").getSelectedKey();
			this.land1 = oBasic.partners.shipfrom.address.country;
			var sRequestQuery = this.getModel().sServiceUrl + "/CommodityCodeSet?$filter=transaction_class eq '" + this.checkmaualapp +
				"' and shippingpoint eq '" + this.shippingpoint + "' and land1 eq '" + this.land1 + "' and profile eq '" + this.sProfile +
				"' and shippingstation eq '" + this.sStation + "'";
			var fnSuccess = function (oData) {
				if (oData.d.results) {
					this.hideBusy();
		...
		onChangeManualShiment: function () {
			var ManualShipment = this.oManualShipment;
			var aIDInput = ["idRequestor", "idshipdescipt"];
			var inputRegex = /^[A-Za-z0-9]{1,4}$/;
			var bCheck = false;
			for (var i = 0; i < aIDInput.length; i++) {
				var sValue = this.byId(aIDInput[i]).getValue().trim();
				if (aIDInput[i] === "idRequestor") {
					if (sValue !== "") {
						if (inputRegex.test(sValue)) {
							ManualShipment.Requestor = sValue;
						} else {
							bCheck = true;
							ManualShipment.Requestor = "";
						}
					}
				} else if (aIDInput[i] === "idshipdescipt") {
					if (sValue !== "") {
						if (inputRegex.test(sValue)) {
							ManualShipment.ShipDesc = sValue;
						} else {
							bCheck = true;
							ManualShipment.ShipDesc = "";
						}
					}

				}
				this.getModel("local").setProperty("/oManualShipment", ManualShipment);
				if (bCheck) {
		...
		onChangMaterialNoDetailPress: function () {
			this.showBusy();
			var oMaterial = this.byId("idMaterial").getValue();
			if (oMaterial !== "") {
				var onMaterialNo = {
					material: oMaterial
				};
				this.getModel("local").setProperty("/oMaterialNoDetail", onMaterialNo);
			}
			var onMaterNo = this.getModel("local").getProperty("/oMaterialNoDetail");
			var oRequestData = this._generateUpdateMaterialNoDetails(onMaterNo);
			this.getModel().create("/ShipmentQuerySet", oRequestData, {
				success: function (oData) {
					this.hideBusy();
					if (oData.ReturnMessages && oData.ReturnMessages.results.length > 0) {
					...
					if (oData.Contents.results.length > 0) {
						delete oData.Contents.results[0].__metadata;
						this.getModel("local").setProperty("/AddContentsDetail", oData.Contents.results[0]);
					}
				}.bind(this),
				error: function (oError) {
					this._handleODataError(oError);
		...
		_generateUpdateMaterialNoDetails: function (oMaterial) {
			var oData = {
				transaction_class: this.checkmaualapp,
				shippingpoint: (this.shippingpoint) ? this.shippingpoint : "",
				shipto: (this.shiptos) ? this.shiptos : "",
		...
		// manual Cost Center
		onManCostCenter: function () {
			var oCostCenter = Utils.getFragment("", "shipment.CostCenter", this);
			oCostCenter.open();
		},

		onCostCenterConfirm: function (oEvent) {
			var oSelectedItemCostCenter = oEvent.getParameter("selectedItem");
			if (oSelectedItemCostCenter) {
				var oValue = oSelectedItemCostCenter.getTitle();
				var oInputCostCenter = this.byId("idCostcenter");
				oInputCostCenter.setValue(oValue);
			}
			oEvent.getSource().getBinding("items").filter([]);
		},

		handleCostCeterValueHelpClose: function (oEvent) {
			var oBinding = oEvent.getSource().getBinding("items");
			oBinding.filter([]);
		},

		// manual SalesDistrict
		onManSalesDistrict: function () {
			var oManualShipmentCat = Utils.getFragment("", "shipment.SalesDistrict", this);
			oManualShipmentCat.open();
		},

		onSalesDistrictConfirm: function (oEvent) {
			var oSelectedItemSalesDistrict = oEvent.getParameter("selectedItem");
			if (oSelectedItemSalesDistrict) {
				var oValue = oSelectedItemSalesDistrict.getTitle();
				var oInputSalesDistrict = this.byId("idSalesDistrict");
				oInputSalesDistrict.setValue(oValue);
			}
			oEvent.getSource().getBinding("items").filter([]);
		},

		handleSalesDistrictValueHelpClose: function (oEvent) {
			var oBinding = oEvent.getSource().getBinding("items");
			oBinding.filter([]);
		},
		// manual shipment cat
		onManShipCategory: function () {
			var oSalesDistrict = Utils.getFragment("", "shipment.ManualShimentCat", this);
			oSalesDistrict.open();
		},

		onShipCategoryConfirm: function (oEvent) {
			var oSelectedShipCategory = oEvent.getParameter("selectedItem");
			if (oSelectedShipCategory) {
				var oValue = oSelectedShipCategory.getTitle();
				var oInputpCategory = this.byId("idManShipCat");
				oInputpCategory.setValue(oValue);
			}
			oEvent.getSource().getBinding("items").filter([]);
		},

		handleCategoryValueHelpClose: function (oEvent) {
			var oBinding = oEvent.getSource().getBinding("items");
			oBinding.filter([]);
		},

		onChangeShipForm: function (oEvent) {
			this.shippingpoint = oEvent.getSource().getSelectedKey();
			var oRequestData = this._generateShippingPoint(this.shippingpoint);
			this.showBusy();
			this.getModel().create("/ShipmentQuerySet", oRequestData, {
				success: function (oData) {
					if (oData) {
		...
		_generateShippingPoint: function (sValueshippingTo) {
			var sActionName = "UpdateManualDetails";
			var oData = {
				transaction_class: this.checkmaualapp,
				shippingpoint: (sValueshippingTo) ? sValueshippingTo : "",
				shipto: (this.shiptos) ? this.shiptos : "",
				basic: this.getModel("local").getProperty("/basic"),
				shipmentid: "",
				profile: (this.sProfile) ? this.sProfile : "",
		...
		AccountHelpSearch: function (oEvent) {
			...
			var aFilters = [];
			if (sValue) {
				aFilters.push(new Filter("SalesOffice", "Contains", sValue));
		...
		_ScanDataReset: function (oValue) {
			var sPath = "/ParcelQuerySet";
			this.getModel().read(sPath, {
				filters: [
					new Filter("input/input_ids", "EQ", this.sInputIDs),
