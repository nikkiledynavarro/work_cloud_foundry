...
sap.ui.define([
	...
	"sap/m/MessageBox",
	"sap/m/MessageToast",
	"com/erpis/shiperp/sls/salesordersls/common/DynamicView"
], function (BaseController, JSONModel, formatter, Device, Filter, Utils, SimpleForm, MessageBox, MessageToast, DynamicView) {
	"use strict";
	return BaseController.extend("com.erpis.shiperp.sls.salesordersls.controller.Detail", {
		...
		onMoreOption: function () {
			...
			var aShipSet = this.getModel("global").getData().ShipSetSet;
			var aMoreOptions = [];
			//fixes Axo 4155
			//can get path from selectedShipsetPath
			var oShipSet = this.getView().getBindingContext("global").getObject();//get Shipset from global
			if (oShipSet) {
				var oMoreOptionData = oShipSet.MoreOptionSet;
				if (oMoreOptionData.results && oMoreOptionData.results.length > 0) {
					aMoreOptions = oMoreOptionData.results;
				}
			}
			if (aMoreOptions && aMoreOptions.length > 0) {
				this._getCarrierMoreOptions(oShipSet, aMoreOptions); //added by Tim 25/8/2021
			}
			...
			if (oData.ShipSetSet) {
				if (oData.ShipSetSet.results.length > 0) {
					...
					if (oData.ShipSetSet.results.length === 1) {
						...
						// oShipset.MoreOptionSet = aOriginMoreOption;//Comment out to update latest carrier more option Tim 28/9/2021
						...
						for (var i = 0; i < oData.ShipSetSet.results.length; i++) {
							for (var j = 0; j < aShipset.length; j++) {
								if (aShipset[j].Counter === oData.ShipSetSet.results[i].Counter) {
									...
									// aShipset[j].MoreOptionSet = aMoreOption;//Comment out to update latest carrier more option Tim 28/9/2021
									break;
								}
							}
						}
		...
		/**
		 * Get Carrier More Option
		 * added by: Tim 25/8/2021
		 * */
		_getCarrierMoreOptions: function (oShipSet, aMoreOptions) {
			//set global
			// this.currentFUPath = sFUItemPath;
			this.oMoreOptionDialog = Utils.getFragment(null, "MoreOptionsDialog", this);
			this.oMoreOptionDialog.destroyContent();
			if (this.sCurrentCarrier) {
				this.oMoreOptionDialog.setTitle("Carrier More Options: " + this.sCurrentCarrier);
			}
			this.hideBusy();
			// Generate More Options Forms
			DynamicView.renderMoreOptionForm(aMoreOptions, this, false);
			// Update current Shipset with the new MoreOptionSet
			// // Update More Option Items to be empty array
			// for (var i = 0; i < oShipSet.MoreOptionSet.results.length; i++) {
			// 	if (oShipSet.MoreOptionSet.results[i].MoreOptionItemSet === null) {
			// 		oShipSet.MoreOptionSet.results[i].MoreOptionItemSet = [];
			// 	}
			// }
			// // Update current Shipset with the new MoreOptionSet
			// var sPath = this.getView().getBindingContext("global").getPath();
			...
			// var sPath = this.getView().getBindingContext("global").getPath();
			// var oShipset = this.getView().getBindingContext("global").getObject();
			// oShipset.MoreOptionSet = oMoreOptionData;
			// this.getModel("global").setProperty(sPath, oShipset);
