/*global location*/
jQuery.sap.require("com.erpis.shiperp.sls.shipewmsls.common.jquery_hotkeys");
sap.ui.define([
	"com/erpis/shiperp/sls/shipewmsls/controller/BaseController",
	"sap/ui/model/json/JSONModel",
	"com/erpis/shiperp/sls/shipewmsls/model/formatter",
	"sap/m/Token",
	"sap/ui/model/Filter",
	"sap/m/MessageBox",
	"sap/m/MessageToast",
	"com/erpis/shiperp/sls/shipewmsls/common/Utils",
	"com/erpis/shiperp/sls/shipewmsls/common/hotkeyInterface",
	"com/erpis/shiperp/sls/shipewmsls/common/DynamicView",
	"com/erpis/shiperp/sls/shipewmsls/common/UploadUtils",
	"com/erpis/shiperp/sls/shipewmsls/common/HttpHelper",
	...
	return BaseController.extend("com.erpis.shiperp.sls.shipewmsls.controller.Main", {

		oLogger: jQuery.sap.log.getLogger("com.erpis.shiperp.sls.shipewmsls.controller.Main"),
		formatter: formatter,
		...
		mapScanData: function (oData) {
			...
			// if (oData.results[0].basic.carrier_data.carrier) {
			// 	this._updateTermOfSale(oData.results[0].basic.carrier_data.carrier);
			// }
			...
			this.getModel().create("/ShipmentQuerySet", oRequestData, {
				success: function (oData) {
					...
					if (oData.autoship.trim() === 'X') {
						this.getModel("local").setProperty("/tickautoship", oData.autoship);
					...
					if (oData.hazmat_err_hu && oData.hazmat_err_hu.results.length > 0) {
						...
						if (!this.ohazmaterrhuDialog) {
							this.ohazmaterrhuDialog = sap.ui.xmlfragment("com.erpis.shiperp.sls.shipewmsls.fragment.Shipmentcontainshazmat", this);
		...
		onDisplayReport: function () {
			if (!this.onOpenReport) {
				this.onOpenReport = sap.ui.xmlfragment("com.erpis.shiperp.sls.shipewmsls.fragment.ReportHazmatMaterial", this);
		...
		onAddDefaultHU: function () {
			if (!this.oDialogDefaultHU) {
				this.oDialogDefaultHU = sap.ui.xmlfragment("com.erpis.shiperp.sls.shipewmsls.fragment.CreateDefaultHUDialog", this);
		...
		onAddNewHU: function () {
			...
			if (!this.oDialogNewHU) {
				this.oDialogNewHU = sap.ui.xmlfragment("com.erpis.shiperp.sls.shipewmsls.fragment.CreateHUDialog", this);
		...
		onShowPricingDetail: function () {
			...
			if (!this.oRatePricingDialog) {
				this.oRatePricingDialog = sap.ui.xmlfragment("com.erpis.shiperp.sls.shipewmsls.fragment.RatePricingsDialog", this);
			...
			var oRatePricingTemplate = sap.ui.xmlfragment("com.erpis.shiperp.sls.shipewmsls.fragment.RatePricingColumnListItem", this);
			var oBindingInfo = {
			...
			this.getModel().create("/ShipmentQuerySet", oRequestData, {
				success: function (oData) {
					...
					if (!this.oRateAnalysisDialog) {
						this.oRateAnalysisDialog = sap.ui.xmlfragment("com.erpis.shiperp.sls.shipewmsls.fragment.RateAnalysisDialog", this);
			...
			this.getModel().create("/ShipmentQuerySet", oRequestData, {
				success: function (oData) {
					...
					if (!this.oConditionTablesDialog) {
						this.oConditionTablesDialog = sap.ui.xmlfragment("com.erpis.shiperp.sls.shipewmsls.fragment.ConditionTablesDialog", this);
		...
		onShowRateDetail: function () {
			...
			if (!this.oRateDetailDialog) {
				this.oRateDetailDialog = sap.ui.xmlfragment("com.erpis.shiperp.sls.shipewmsls.fragment.RateDetailsDialog", this);
				this.getView().addDependent(this.oRateDetailDialog);
			}
			var oObject = sap.ui.getCore().byId("tableRates").getSelectedItem().getBindingContext("local").getObject();
			var oRateDetailTemplate = sap.ui.xmlfragment("com.erpis.shiperp.sls.shipewmsls.fragment.RateDetailColumnListItem", this);
			var oBindingInfo = {
		...
		onPackBundle: function () {
			if (!this.oPackBundleDialog) {
				this.oPackBundleDialog = sap.ui.xmlfragment("com.erpis.shiperp.sls.shipewmsls.fragment.PackBundleDialog", this);
		...
		onPackBundleConfirm: function () {
			...
			// if (sap.ui.getCore().byId("txtHUNoBundle").getValue() === "") {
			// 	sap.ui.getCore().byId("txtHUNoBundle").setValue("1");
			// }
			this._packBundle();
		},
		...
		onUnpack: function () {
			...
			if (aSelectedHUs[0].getBindingContext("local").getObject().hu_items && aSelectedHUs[0].getBindingContext("local").getObject().hu_items
				.results.length === 0) {
		...
		_unPackItems: function (aItemRepack) {
			...
			this.getModel().create("/ShipmentQuerySet", oRequestData, {
				success: function (oData) {
					...
					// var aHUListRemove = aHUList.filter(function (data) {
					// 	return data.Outbhu !== parseInt(oData.PackScanner.results[0].SourceId, 10);
					// });
					this.getModel("local").setProperty("/DestHU", aHUListRemove);
					if (!this.oRepackOutboundHU) {
						this.oRepackOutboundHU = sap.ui.xmlfragment("com.erpis.shiperp.sls.shipewmsls.fragment.RePackItemDialog", this);
			...
			if (sHuNum) {
				...
				if (oMassUpdateScale) {
					...
					for (var property in oMassUpdateScale) {
						...
						// }
						// else if (property === "length" || property === "width" && property === "height" || property === "dimensionunit") {
		...
		onSerialPress: function () {
			if (!this.oSerialDialog) {
				this.oSerialDialog = sap.ui.xmlfragment("com.erpis.shiperp.sls.shipewmsls.fragment.SerialDialog", this);
		...
		onReprint: function () {
			...
			var oReprintTemplate = sap.ui.xmlfragment("com.erpis.shiperp.sls.shipewmsls.fragment.ReprintColumnListItem", this);
			// var oFilter
		...
		*/
		...
		onShowFDXGHoldLocationDetail: function () {
			...
			if (!this.oDialogFDXGHoldLocationDetail) {
				this.oDialogFDXGHoldLocationDetail = sap.ui.xmlfragment(
					"com.erpis.shiperp.sls.shipewmsls.fragment.carrier.FedExGroundHoldAtLocationDetailDialog",
					this);
		...
		onShowFDXEHoldLocationDetail: function () {
			...
			if (!this.oDialogFDXEHoldLocationDetail) {
				this.oDialogFDXEHoldLocationDetail = sap.ui.xmlfragment(
					"com.erpis.shiperp.sls.shipewmsls.fragment.carrier.FedExHoldAtLocationDetailDialog",
					this);
		...
		onShowFDXGCODRecipientDetail: function () {
			...
			if (!this.oDialogCODFDXGRecipientDetail) {
				this.oDialogCODFDXGRecipientDetail = sap.ui.xmlfragment(
					"com.erpis.shiperp.sls.shipewmsls.fragment.carrier.FedExGroundCODRecipientDetailDialog",
					this);
		...
		onShowFDXECODRecipientDetail: function () {
			...
			if (!this.oDialogCODFDXERecipientDetail) {
				this.oDialogCODFDXERecipientDetail = sap.ui.xmlfragment(
					"com.erpis.shiperp.sls.shipewmsls.fragment.carrier.FedExCODRecipientDetailDialog",
					this);
		...
		_getFreightCost: function (sWhichAction) {
			...
			this.getModel().create("/ShipmentQuerySet", oRequestData, {
				success: function (oData) {
					...
					if (!this.oRateDialog) {
						this.oRateDialog = sap.ui.xmlfragment("com.erpis.shiperp.sls.shipewmsls.fragment.RatesDialog", this);
		...
		getFinalHazmatOptionData: function (oHazmatOptions) {
			if (oHazmatOptions.getContent().length > 0) {
				var oHazmatOptionsForm = oHazmatOptions.getContent()[0];
