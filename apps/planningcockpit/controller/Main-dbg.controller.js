/*global location*/
jQuery.sap.require("com.erpis.shiperp.planningcockpit.common.jquery_hotkeys");
sap.ui.define([
	"com/erpis/shiperp/planningcockpit/controller/BaseController",
	"sap/ui/model/json/JSONModel",
	"com/erpis/shiperp/planningcockpit/model/formatter",
	"sap/m/Token",
	"sap/ui/model/Filter",
	"sap/m/MessageBox",
	"sap/m/MessageToast",
	"com/erpis/shiperp/planningcockpit/common/Utils",
	"com/erpis/shiperp/planningcockpit/common/hotkeyInterface"
], function (BaseController, JSONModel, formatter, Token, Filter, MessageBox, MessageToast, Utils, HotkeyInterface) {
	"use strict";

	return BaseController.extend("com.erpis.shiperp.planningcockpit.controller.Main", {

		oLogger: jQuery.sap.log.getLogger("com.erpis.shiperp.planningcockpit.controller.Main"),
		formatter: formatter,
		...
		oFUTable: null, // Freight Units table.
		oFOTable: null, // Freight Orders table.
		...
		_onObjectMatched: function (oEvent) {
			...
			this.oFUTable = this.byId("tableFU");
			this.oFOTable = this.byId("tableFO");
			...
			this.getModel().read(sPath, {
				...
				success: function (oData) {
					if (oData.results.length !== 0 && oData.results[0].basic.carrier_data.carrier !== "") {
						...
						this.byId("groupmstrip").setVisible(true);
		...
		onAddDefaultHU: function () {
			if (!this.oDialogDefaultHU) {
				this.oDialogDefaultHU = sap.ui.xmlfragment("com.erpis.shiperp.planningcockpit.fragment.CreateDefaultHUDialog", this);
		...
		onAddNewHU: function () {
			if (!this.oDialogNewHU) {
				this.oDialogNewHU = sap.ui.xmlfragment("com.erpis.shiperp.planningcockpit.fragment.CreateHUDialog", this);
		...
		onCreateFreightOrder: function () {
			this._createFreightOrder();
		},
		...
		onBtnPressRefChange: function () {
			this._oRefDialog = Utils.getFragment("", "planningcockpit.DialogRefDisplay", this);
		...
		onOpenHUItemsDialog: function (oEvent) {
			...
			if (!this._oHUItemsDialog) {
				this._oHUItemsDialog = sap.ui.xmlfragment("com.erpis.shiperp.planningcockpit.fragment.packing.HUItemsDialog", this);
		...
		//Adapt filter - Henry 21/05/2020
		onFilter: function () {
			this.oAdaptFilterFragment = Utils.getFragment("", "packing.AdaptFilter", this);
			this.oAdaptFilterFragment.open();
		},

		onCloseAdaptFilter: function () {
			this.oAdaptFilterFragment.close();
		},
		...
		onShowPricingDetail: function () {
			...
			if (!this.oRatePricingDialog) {
				this.oRatePricingDialog = sap.ui.xmlfragment("com.erpis.shiperp.planningcockpit.fragment.RatePricingsDialog", this);
			...
			var oRatePricingTemplate = sap.ui.xmlfragment("com.erpis.shiperp.planningcockpit.fragment.RatePricingColumnListItem", this);
			var oBindingInfo = {
		...
		onShowRateDetail: function () {
			...
			if (!this.oRateDetailDialog) {
				this.oRateDetailDialog = sap.ui.xmlfragment("com.erpis.shiperp.planningcockpit.fragment.RateDetailsDialog", this);
				this.getView().addDependent(this.oRateDetailDialog);
			}
			var oObject = sap.ui.getCore().byId("tableRates").getSelectedItem().getBindingContext("local").getObject();
			var oRateDetailTemplate = sap.ui.xmlfragment("com.erpis.shiperp.planningcockpit.fragment.RateDetailColumnListItem", this);
			var oBindingInfo = {
		...
		onAssignFUToFO: function () {
			var oFUTable = this.byId("tableFU");
			var oFOTable = this.byId("tableFO");
			
			var aSelectedFUs = oFUTable.getSelectedItems();
			if (aSelectedFUs.length === 0) {
				MessageBox.error(this.oBundle.getText("SelectItemPack"));
				return;
			}
			var aSelectedFOs = oFOTable.getSelectedItems();
			if (aSelectedFOs.length !== 1) {
				MessageBox.error(this.oBundle.getText("SelectHUPack"));
				return;
			}

			this._assignFUToFO(aSelectedFUs[0].getCells()[0].getText(), aSelectedFOs[0].getCells()[0].getText());
		},
		...
		onSerialPress: function () {
			if (!this.oSerialDialog) {
				this.oSerialDialog = sap.ui.xmlfragment("com.erpis.shiperp.planningcockpit.fragment.SerialDialog", this);
		...
		onReprint: function () {
			...
			var oReprintTemplate = sap.ui.xmlfragment("com.erpis.shiperp.planningcockpit.fragment.ReprintColumnListItem", this);
			// var oFilter
		...
		onShowFDXGHoldLocationDetail: function () {
			...
			if (!this.oDialogFDXGHoldLocationDetail) {
				this.oDialogFDXGHoldLocationDetail = sap.ui.xmlfragment(
					"com.erpis.shiperp.planningcockpit.fragment.carrier.FedExGroundHoldAtLocationDetailDialog",
					this);
		...
		onShowFDXEHoldLocationDetail: function () {
			...
			if (!this.oDialogFDXEHoldLocationDetail) {
				this.oDialogFDXEHoldLocationDetail = sap.ui.xmlfragment(
					"com.erpis.shiperp.planningcockpit.fragment.carrier.FedExHoldAtLocationDetailDialog",
					this);
		...
		onShowFDXGCODRecipientDetail: function () {
			...
			if (!this.oDialogCODFDXGRecipientDetail) {
				this.oDialogCODFDXGRecipientDetail = sap.ui.xmlfragment(
					"com.erpis.shiperp.planningcockpit.fragment.carrier.FedExGroundCODRecipientDetailDialog",
					this);
		...
		onShowFDXECODRecipientDetail: function () {
			...
			if (!this.oDialogCODFDXERecipientDetail) {
				this.oDialogCODFDXERecipientDetail = sap.ui.xmlfragment(
					"com.erpis.shiperp.planningcockpit.fragment.carrier.FedExCODRecipientDetailDialog",
					this);
		...
		/*Freight Order*/
		onCreateFreightOrderDialog: function () {
			if (!this.oDialogCreateFreightOrder) {
				this.oDialogCreateFreightOrder = sap.ui.xmlfragment("com.erpis.shiperp.planningcockpit.fragment.packing.CreateFreightOrder", this);
				this.getView().addDependent(this.oDialogCreateFreightOrder);
			}
			this.oDialogCreateFreightOrder.open();
		},

		onCreateFreightOrderClose: function () {
			this.oDialogCreateFreightOrder.close();
		},
		/*End Freight Order*/
		/*Update routing*/
		onUpdateRoutingDialog: function () {
			if (!this.oDialogUpdateRouting) {
				this.oDialogUpdateRouting = sap.ui.xmlfragment("com.erpis.shiperp.planningcockpit.fragment.packing.UpdateRouting", this);
				this.getView().addDependent(this.oDialogUpdateRouting);
			}
			this.oDialogUpdateRouting.open();
		},

		onUpdateUpdateRoutingClose: function () {
			this.oDialogUpdateRouting.close();
		},
		/*End Update Routing*/
		/*Request routing*/
		onRequestRoutingDialog: function () {
			if (!this.oDialogRequestRouting) {
				this.oDialogRequestRouting = sap.ui.xmlfragment("com.erpis.shiperp.planningcockpit.fragment.packing.RequestRouting", this);
				this.getView().addDependent(this.oDialogRequestRouting);
			}
			this.oDialogRequestRouting.open();
		},

		onUpdateRequestRoutingClose: function () {
			this.oDialogRequestRouting.close();
		},
		/*End Request Routing*/
		/*Request For Pickup*/
		onRequestForPickupDialog: function () {
			if (!this.oDialogRequestForPickup) {
				this.oDialogRequestForPickup = sap.ui.xmlfragment("com.erpis.shiperp.planningcockpit.fragment.packing.RequestForPickup", this);
				this.getView().addDependent(this.oDialogRequestForPickup);
			}
			this.oDialogRequestForPickup.open();
		},

		onRequestForPickupClose: function () {
			this.oDialogRequestForPickup.close();
		},
		/*End Request For Pickup*/
		...
		_resetData: function () {
			...
			this.byId("iconTabInternational").setVisible(false);
			this.byId("iconTabImporter").setVisible(false);
			this.byId("groupmstrip").setVisible(false);
			...
			this.getModel().create("/ShipmentQuerySet", oRequestData, {
				success: function (oData) {
					...
					if (!this.oRateAnalysisDialog) {
						this.oRateAnalysisDialog = sap.ui.xmlfragment("com.erpis.shiperp.planningcockpit.fragment.RateAnalysisDialog", this);
		...
		_getFreightCost: function (sWhichAction) {
			...
			this.getModel().create("/ShipmentQuerySet", oRequestData, {
				success: function (oData) {
					...
					if (!this.oRateDialog) {
						this.oRateDialog = sap.ui.xmlfragment("com.erpis.shiperp.planningcockpit.fragment.RatesDialog", this);
		...
		_createFreightOrder: function () {
			...
			this.showBusy();
			this.getModel().create("/xSERPTMxI_FRTORDER", 
				{
					carrier: sap.ui.getCore().byId("cbCarrier").getValue(),
					doctype: sap.ui.getCore().byId("cbDocType").getValue(),
					sourceloc: sap.ui.getCore().byId("cbSourceLocation").getValue(),
					destloc:   sap.ui.getCore().byId("cbDestLocation").getValue(),
					departuredt: sap.ui.getCore().byId("dpDepDate").getValue() + sap.ui.getCore().byId("tpDepTime").getValue(),
					arrivaldt: sap.ui.getCore().byId("dpArrDate").getValue() + sap.ui.getCore().byId("tpArrTime").getValue()
				},
				{
					success: function () {
						this._refreshPackingData();
						MessageToast.show(this.oBundle.getText("CreateFOSuccess"));
						this.oDialogCreateFreightOrder.close();
					}.bind(this),
					error: function (oError) {
						this._handleODataError(oError);
		...
		_assignFUToFO: function (fu, fo) {
			this.showBusy();
			this.getModel().callFunction("/AssignFUToFO", {
				urlParameters: {
					FU: fu,
					FO: fo
				},
				success: function () {
					this.byId("tableFU").getBinding("items").refresh();
					this.byId("tableFO").getBinding("items").refresh();
					this.hideBusy();
					MessageToast.show(this.oBundle.getText("PackMaterialSuccess"));
				}.bind(this),
				error: function (oError) {
