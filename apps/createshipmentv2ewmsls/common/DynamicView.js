sap.ui.define(["sap/ui/layout/form/SimpleForm", "com/erpis/shiperp/hr7/parcelewm/common/ControlUtils",
	...
	return {
			...
			if (aSourceFields.length > 0) {
				...
				for (var i = 0; i < aSourceFields.length; i++) {
					...
					if (aSourceFields[i].field_type !== "10") {
						var skeyCountry = oController.getModel("local").getProperty("/basic/partners/shipfrom/address/country");
					...
					if (aSourceFields[i].field_value2 !== "") {
						if (aSourceFields[i].field_value2.column.length > 0) {
							...
							var oTitle = new sap.m.Title({
								text: aSourceFields[i].field_desc,
			...
			if (sLevel === "PACKAGE") {
				for (var i = 0; i < oSource.length; i++) {
					...
					if (packageFieldName === sFieldName) {
						...
						// oSource.carrierMoreOption.results[i].field_value = sValue2;
						break;
					}
				}
		...
		_updateMoreOptionData: function (oSource, sFieldName, sValue1, sValue2, sLevel) {
			...
			//process for package
			if (sLevel === "PACKAGE") {
				for (var i = 0; i < oSource.carrierMoreOption.results.length; i++) {
					var packageFieldName = oSource.carrierMoreOption.results[i].field_name;
					packageFieldName = packageFieldName.replaceAll("PACKAGE_", "").replaceAll("SHIPMENT_", "");
					if (packageFieldName === sFieldName) {
						oSource.carrierMoreOption.results[i].field_value = sValue1;
						// oSource.carrierMoreOption.results[i].field_value = sValue2;
						break;
					}
				}
		...
		removePrefixBeforeExecuteInternational: function (aSources) {
			aSources.forEach(function (item) {
				item.field_name = item.field_name.replaceAll("PACKAGE_", "").replaceAll("SHIPMENTX_", "");
			});
			return aSources;
		},
		removePrefixPackage: function (oController) {
			...
			aFreightUnits.forEach(function (fuItem) {
				fuItem.carrierMoreOption.results = oThis.removePrefixBeforeExecute(fuItem.carrierMoreOption.results);
			});
